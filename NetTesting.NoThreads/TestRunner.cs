...
namespace NetTesting
{
    ...
    {
        private static Dictionary<Type, List<MethodInfo>> _types;
        ...
        protected override void InternalInit(Assembly targetAssembly = null)
        {
            ...
            _types = new Dictionary<Type, List<MethodInfo>>();
            foreach(Type type in _targetAssembly.GetTypes())
            {
                try
                {
                    if (IsTextFixture(type))
                    {
                        _types.Add(type, new List<MethodInfo>());
